...
export default function TrendingElectronics() {
  ...
  React.useEffect(() => {
    ...
    if (selectedTrend.rating > rated.length) {
      rated.push(
  ...
  const gallery = trendData.map((item) => {
    return (
      <span
        key={item.id}
        className={item.id === selectedTrend.id ? "gallery active" : "gallery"}
        style={{ backgroundImage: `url(${item.img})` }}
        onMouseEnter={() => selecteTrend(item.id)}
      ></span>
    );
  });
    ...
    <Container className="py-5">
      <Row>
        <Col
          ...
          sm={12}
          md={12}
          className="d-flex  justify-content-center align-items-center gap-lg-5 gap-3"
        >
          {gallery}
        </Col>
        <Col lg={6} sm={12} md={12} className="text-center py-5">
          <img
            src={selectedTrend.img}
            className="img-fluid"
            alt={selectedTrend.id}
          ></img>
        </Col>
        <Col lg={6} sm={12} className="px-2 py-2 py-lg-5 d-flex flex-column">
          ...
          <div>
            {rating.map((item) => {
              return (
                <span key={Math.random()} title={selectedTrend.rating}>
                  {item}
                </span>
              );
            })}
            <span>
              -<b> {selectedTrend.raters} Ratings </b>
            </span>
            <hr />
            <ul>

...
export default function ProductInfo({ allProducts, addproduct, myCart }) {
  ...
  function rating(rate) {
    ...
    for (let i = 0; i < Math.floor(rate); i++) {
      rated.push(
        <i className="fa-solid fa-star" style={{ color: "gold" }} key={i}></i>
      );
    }
        ...
        {theProduct ? (
          <Row>
            <Col
              ...
              style={{ backgroundImage: `url(${theProduct.fullImg})` }}
            ></Col>
            <Col lg={6} className="px-4 py-4">
              <h3 style={{ color: "#242b2f" }}>
                <b>{theProduct.fullName}</b>
              </h3>
              <span title={theProduct.rating}>
                ...
                <>
                  <div className="prices d-flex align-items-center  flex-wrap">
                    ...
                    <span className="real-price px-3">
                      ...
                      <span className="decimal real">
                        {" "}
                        {theProduct.price.toString().split(".")[1]}¢{" "}
                      </span>
                    </span>
                    <span className="after-price px-2">
                      <b>
                        ${Math.floor(theProduct.price - theProduct.sale)}
                  ...
                  <div className="availability py-3">
                    {theProduct.left <= 5 ? (
                      <span>
                        ...
                        <b>
                          {" "}
                          <span style={{ color: "red" }}> Note : </span> there
                          is Only {theProduct.left} Items Left in the Stock -
                          Order soon .{" "}
                        </b>
                      </span>
                    ) : (
                      <span className="bold fs-3" style={{ color: "green" }}>
                        {" "}
                        <b>In Stock </b>
                      </span>
                    )}
              ...
              <div className="buy ">
                {theProduct.available ? addremovebutton() : null}
              </div>
            </Col>
          </Row>
        ) : null}

...
function App() {
  ...
  function addproduct(product) {
    ...
    setMyCart((previtems) =>
      previtems.map((item) => {
        return item.id === product.id ? { ...item, count: 1 } : item;
      })
    );
  }
  ...
  function editsize(id, value) {
    setMyCart((prevState) =>
      prevState.map((item) => {
        return item.id === id ? { ...item, size: value } : item;
      })
    );
  }
  ...
  function search(value) {
    ...
    if (value) {
      ...
      allProducts.slice(4).forEach((items) => {
        if (
          items.title.includes(value) ||
          items.title.includes(value.toUpperCase()) ||
          items.title.includes(
            `${value[0].toUpperCase()}${value.substring(1)}`
          ) ||
          items.title.includes(
            `${value[0].toUpperCase()}${value[1].toLowerCase()}`
          )
        ) {
          search.push(items);
        }
  ...
  return (
    <>
      ...
      <Routes>
        ...
        />
        <Route
          path="jewelery/product/:productID"
          element={
            <ProductInfoJ
              allProducts={allProducts}
              addproduct={addproduct}
        ...
        <Route
          path="discover"
          element={<Discover search={search} searchres={searchres} />}
        />
        <Route
          path="mycart"
          element={
